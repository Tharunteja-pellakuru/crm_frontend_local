  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (onAddLead) {
        ...
        setFormData({ name: "", company: "", email: "", phone: "", status: "Lead", leadType: "Hot", industry: "", notes: "", projectName: "", projectStatus: "Planning", projectCategory: 1, projectPriority: "High", projectDescription: "", country: "India", countryCode: "+91" });
                      ...
                      <td className="px-6 py-5 text-right">
                        <div className="flex justify-end gap-3" onClick={(e) => e.stopPropagation()}>
                          {lead.status !== "Dismissed" && (
                            <button onClick={(e) => { e.stopPropagation(); handleOpenFollowUpModal(lead); }} className="w-[34px] h-[34px] flex items-center justify-center bg-indigo-50/50 border border-indigo-100 rounded-[10px] text-indigo-500 hover:bg-indigo-100 transition-all active:scale-90 shadow-sm relative group/btn">
                              <BellRing size={16} />
                          ...
                          {leadView === "Converted" ? (
                            ...
                            <>
                              ...
                              {onDismissLead && (lead.status === "Lead" || lead.isConverted) && lead.status !== "Dismissed" && (
                                <button onClick={(e) => { e.stopPropagation(); onDismissLead(lead.lead_id); }} className="w-[34px] h-[34px] flex items-center justify-center bg-[#FFF9ED] border border-[#FDE68A] rounded-[10px] text-[#F59E0B] hover:text-[#D97706] hover:border-[#FCD34D] transition-all active:scale-90 shadow-sm relative group/btn">
                                  <UserX size={16} />
                              ...
                              {onRestoreLead && lead.status === "Dismissed" && (
                                <div className="flex gap-2">
                                  <button onClick={(e) => { e.stopPropagation(); onRestoreLead(lead.lead_id); }} className="w-[34px] h-[34px] flex items-center justify-center bg-blue-50/50 border border-blue-100 rounded-[10px] text-blue-500 hover:bg-blue-100 transition-all active:scale-90 shadow-sm relative group/btn">
                                    <RotateCcw size={16} />
                                  ...
                                  {onDeleteLead && (
                                    <button onClick={(e) => { e.stopPropagation(); setLeadToDelete(lead); setShowDeleteModal(true); }} className="w-[34px] h-[34px] flex items-center justify-center bg-[#FEF2F2] border border-[#FECACA] rounded-[10px] text-[#EF4444] hover:text-[#DC2626] hover:border-[#FCA5A5] transition-all active:scale-90 shadow-sm relative group/btn">
                                      <Trash2 size={16} />
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className={`px-3 py-1 rounded-lg text-[14px] font-bold border flex items-center gap-1.5 shadow-sm ${status.className}`}>
                    {status.icon}{status.label}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-slate-100">
                  <button onClick={(e) => { e.stopPropagation(); onSelectLead(lead); }} className="flex items-center gap-1 text-[12px] font-bold text-secondary uppercase tracking-widest hover:text-secondary/80 transition-colors">
                    View Details <ChevronRight size={14} />
                  </button>
                  <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                    {lead.status !== "Dismissed" && (
                      <button onClick={(e) => { e.stopPropagation(); handleOpenFollowUpModal(lead); }} className="p-2 bg-indigo-50 text-indigo-600 border border-indigo-100 rounded-lg hover:bg-indigo-100 transition-all active:scale-90"><BellRing size={16} /></button>
                    )}
                    {leadView === "Converted" ? (
                      <button
                        onClick={(e) => {
                      ...
                      <>
                        {onOnboardLead && lead.status === "Lead" && (
                          <button
                            onClick={(e) => {
                              ...
                              const initialCountry = lead.country || "";
                              const countryCurrency = countryToCurrency[initialCountry]?.code || "";
                              setOnboardingData({ name: lead.name, email: lead.email, phone: lead.phone, clientType: "New", status: "Active", projectName: "", projectDescription: "", onboardingDate: new Date().toISOString().split("T")[0], deadline: "", scopeDocument: "", projectStatus: "Planning", projectCategory: lead.projectCategory || 1, projectPriority: "High", clientStatus: "Active", currency: lead.currency || countryCurrency || "", country: initialCountry, state: lead.state || "" });
                              setShowOnboardModal(true);
                            }}
                        ...
                        {onDismissLead && (lead.status === "Lead" || lead.isConverted) && lead.status !== "Dismissed" && (
                          <button onClick={(e) => { e.stopPropagation(); onDismissLead(lead.lead_id); }} className="p-2 bg-amber-50 text-amber-600 border border-amber-100 rounded-lg hover:bg-amber-100 transition-all active:scale-90"><UserX size={16} /></button>
                        )}
                        ...
                        {onRestoreLead && lead.status === "Dismissed" && (
                          <div className="flex gap-2">
                            <button onClick={(e) => { e.stopPropagation(); onRestoreLead(lead.lead_id); }} className="p-2 bg-blue-50 text-blue-600 border border-blue-100 rounded-lg hover:bg-blue-100 transition-all active:scale-90"><RotateCcw size={16} /></button>
                            {onDeleteLead && (
                              <button onClick={(e) => { e.stopPropagation(); setLeadToDelete(lead); setShowDeleteModal(true); }} className="p-2 bg-rose-50 text-rose-600 border border-rose-100 rounded-lg hover:bg-rose-100 transition-all active:scale-90"><Trash2 size={16} /></button>
                            )}
                          </div>
                        )}
                      </>
      ...
      {showEditConvertedModal && createPortal(
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[99999] flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
          ...
          <div className="relative z-10 bg-white w-full max-w-lg rounded-3xl shadow-2xl border border-slate-100 overflow-hidden animate-pop flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleEditConvertedSubmit} className="p-6 space-y-4 overflow-y-auto no-scrollbar">
              <div className="space-y-1.5">
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-2">
                    <select className="flex-1 px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-[#18254D] focus:outline-none focus:ring-4 focus:ring-[#18254D]/5 focus:border-[#18254D]/30" value={followUpData.timeHour} onChange={(e) => setFollowUpData({ ...followUpData, timeHour: e.target.value })}>
                    ...
                    <select className="flex-1 px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-[#18254D] focus:outline-none focus:ring-4 focus:ring-[#18254D]/5 focus:border-[#18254D]/30" value={followUpData.timeMinute || "00"} onChange={(e) => setFollowUpData({ ...followUpData, timeMinute: e.target.value })}>
                      {Array.from({ length: 60 }, (_, i) => i).map((m) => (<option key={`lead-follow-min-${m}`} value={String(m).padStart(2, "0")}>{String(m).padStart(2, "0")}</option>))}
                    </select>
                    <select className="w-20 px-2 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-[#18254D] focus:outline-none focus:ring-4 focus:ring-[#18254D]/5 focus:border-[#18254D]/30" value={(followUpData.timePeriod || "PM").toUpperCase()} onChange={(e) => setFollowUpData({ ...followUpData, timePeriod: e.target.value })}>
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="flex gap-2">
                        <select className="flex-1 px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-[#18254D] focus:outline-none focus:ring-4 focus:ring-[#18254D]/5 focus:border-[#18254D]/30" value={followUpData.completionHour} onChange={(e) => setFollowUpData({ ...followUpData, completionHour: e.target.value })}>
                          {Array.from({ length: 12 }, (_, i) => i + 1).map((h) => (<option key={`comp-hour-${h}`} value={String(h).padStart(2, "0")}>{String(h).padStart(2, "0")}</option>))}
                        </select>
                        <select className="flex-1 px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-[#18254D] focus:outline-none focus:ring-4 focus:ring-[#18254D]/5 focus:border-[#18254D]/30" value={followUpData.completionMinute} onChange={(e) => setFollowUpData({ ...followUpData, completionMinute: e.target.value })}>
                          {Array.from({ length: 60 }, (_, i) => i).map((m) => (<option key={`comp-min-${m}`} value={String(m).padStart(2, "0")}>{String(m).padStart(2, "0")}</option>))}
                        </select>
                        <select className="w-20 px-2 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-[#18254D] focus:outline-none focus:ring-4 focus:ring-[#18254D]/5 focus:border-[#18254D]/30" value={followUpData.completionPeriod} onChange={(e) => setFollowUpData({ ...followUpData, completionPeriod: e.target.value })}>
                          <option value="AM">AM</option>
                          <option value="PM">PM</option>
                        </select>
                      </div>
